...
```
```{r}
# try to check the number of k-menas
fviz_nbclust(weather_norm,
             method = "wss", # within the sum of squares
             k.max = 10,
             FUNcluster = kmeans
             )
...
```
```{r}
# let's try 9 here. Because I want to make this (between_SS / total_SS =  66.1 %) higher, but not to less efficient.
...
ggsave("../output/clustervalidation9.png",
       width = 10,
       height = 20)  
...
```
When k=9, the plot is worse because the cluster 8 and 9 has big overlap with each other,and cluster 3 and 4 do as well. So it's bad to make 9 clusters for them. 
And you can also get the same idea based on the boxplot of distribution of each variables against cluster. There boxplot also overlap a lot, I think its because too many clusters has been chosen.

Four is much better because the clusters have much clearer boundaries. 
